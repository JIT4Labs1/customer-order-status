...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* ── Inventory Opportunities: per-vendor INV columns in Open Vendor POs ── */
  @keyframes fopPulse {
  ...
  .fop-x { color:#c62828; font-weight:700; font-size:13px; }
  ...
  /* Scrollable inventory grid with headers pinned to the top of the scroll box. */
  .iop-scroll { max-height:62vh; overflow:auto; border:1px solid #e3e9f0; border-radius:8px; }
  .iop-scroll thead th { position:sticky; top:0; z-index:3; background:#0D2B45; }
  .iop-scroll table { border-collapse:separate; border-spacing:0; }
  .iop-scroll tbody td { border-bottom:1px solid #eef2f6; }
  /* Invoice Check tab */
...
// One inventory column per vendor whose file has been uploaded — "PMA INV",
// "Allora INV", … built at render time so adding or removing a file just adds or
// removes a column. Appended after ETA in the Open Vendor POs table.
function ioppInvVendors(){
  var fs=(IOPP&&IOPP.files)||[], seen={}, out=[];
...
function colsVendorDyn(){
  var cols=COLS_VENDOR.slice(), vs=ioppInvVendors();
...
function renderVendorPanel(){
  ...
  var vcols=colsVendorDyn(), invCols=vcols.slice(COLS_VENDOR.length);
  var ncol=vcols.length, body='';
  function invCells(row){
    var h='';
    for(var ci=0;ci<invCols.length;ci++) h+='<td class="c">'+fopVenCell(row, invCols[ci].vendor)+'</td>';
    return h;
  }
  for(var gi=0;gi<order.length;gi++){
    var grp=groups[order[gi]];
...
// vendor PO shows a flashing green quantity in that vendor's "<Vendor> INV" column.
...
// Short display label for the narrow per-vendor INV columns: the distinctive first word.
// "Allora Biotech LLC" -> "Allora" · "PMA Services" -> "PMA" ·
// "ClearChem Diagnostics, Inc" -> "ClearChem" · "ALDX HOLDING CORPORATION" -> "ALDX".
// Full names are kept everywhere else (tooltip, Inventory Opportunities tab).
function ioppShortVen(s){
  var t=String(s==null?'':s).replace(/[.](xlsx|xls|csv)$/i,'').trim();
  if(!t) return '?';
  var parts=t.split(/[ ,/_-]+/);
  for(var i=0;i<parts.length;i++){
    var w=parts[i].replace(/[^A-Za-z0-9&+]/g,'');
    if(!w) continue;
    var low=w.toLowerCase();
    // skip leading filler so "The Acme Group" -> "Acme"
    if(low==='the'||low==='a'||low==='an') continue;
    return w;
  }
  return t.slice(0,12);
}
...
// Total qty one vendor holds of a SKU, summed across ALL its lots.
// Returns 0 when that vendor has none (or the SKU is in no uploaded file at all).
function ioppQtyVen(sku, vendorName){
  var hit=ioppFor(sku); if(!hit) return 0;
  var vn=ioppVenNorm(vendorName), q=0;
  for(var i=0;i<hit.srcs.length;i++){
    if(ioppVenNorm(hit.srcs[i].vendor)===vn) q+=hit.srcs[i].qty;
  }
  return q;
}
// A per-vendor inventory cell ("PMA INV" / "Allora INV") in Open Vendor POs.
// Positive stock flashes green with the summed quantity; nothing in stock is a red ✗.
function fopVenCell(r, vendorName){
  var sku=r&&r.sku, q=ioppQtyVen(sku, vendorName);
  if(q<=0){
    return '<span class="fop-x" title="'+escapeHtml(vendorName+' has no stock of '+(sku||'this SKU'))+'">&#10007;</span>';
  }
  var hit=ioppFor(sku), lots=[];
  for(var i=0;i<hit.srcs.length;i++){
    var s=hit.srcs[i];
    if(ioppVenNorm(s.vendor)!==ioppVenNorm(vendorName)) continue;
    lots.push(fmtQty(s.qty)+(s.lot?(' — lot '+s.lot):'')+(s.exp?(' exp '+s.exp):''));
  }
  var tip=vendorName+': '+fmtQty(q)+' in stock'+
          (lots.length>1?(' across '+lots.length+' lots'):'')+IOPP_NL+lots.join(IOPP_NL);
  return '<span class="fop-txt" title="'+escapeHtml(tip)+'" '+
         'onclick="setMode(String.fromCharCode(105,111,112,112))">'+fmtQty(q)+'</span>';
}
// (kept) Alternative-source summary — still used by the Inventory Opportunities count.
function fopCell(r, poVendor){
  ...
  for(var k=0;k<order.length;k++){
    var v=byVen[order[k]];
    if(k<2) shown.push(escapeHtml(ioppShortVen(v.vendor))+' <span class="fop-q">'+fmtQty(v.qty)+'</span>');
...
// Some vendor sheets pack the lot and expiry into one free-text cell, e.g. a
// "Details" column reading "Lot#: 20970 Exp Date: 11/30/2027". These pull the two
// values back out. Built with new RegExp (no backslash escapes) because this whole
// script is emitted from a non-raw Python string — [^A-Za-z0-9] stands in for the
// separators (space, #, :, etc).
var RE_IOPP_LOT  = new RegExp('lot[^A-Za-z0-9]*([A-Za-z0-9][A-Za-z0-9-]*)','i');
var RE_IOPP_EXPD = new RegExp('exp[a-z]*[^A-Za-z0-9]*date[^A-Za-z0-9]*([0-9][0-9/.-]*)','i');
var RE_IOPP_EXP  = new RegExp('exp[a-z]*[^A-Za-z0-9]*([0-9][0-9/.-]*)','i');
var RE_IOPP_HASNUM = new RegExp('[0-9]');
function _ioppFromDetails(txt, rec){
  var s=String(txt==null?'':txt); if(!s) return;
  // A lot value must contain a digit — stops prose like "no lot info here" from
  // capturing the next word as a lot number.
  if(!rec.lot){ var lm=s.match(RE_IOPP_LOT);
    if(lm && lm[1] && RE_IOPP_HASNUM.test(lm[1])) rec.lot=lm[1].slice(0,40); }
  if(!rec.exp){
    var em=s.match(RE_IOPP_EXPD)||s.match(RE_IOPP_EXP);
...
// Typing must NOT re-render the whole panel — that replaces the <input> and the
// caret jumps out, forcing a click before every keystroke. Only the rows redraw.
function ioppSearch(v){ ioppQ=String(v||'').toLowerCase(); ioppRenderRows(); }
// Sort state for the inventory grid. Default: SKU ascending.
var ioppSort={key:'sku', dir:1};
var IOPP_COLS=[
  {k:'flag', label:'',            t:'num',  c:true},
  {k:'sku',  label:'SKU',         t:'str'},
  {k:'desc', label:'Description', t:'str'},
...
function ioppSortBy(i){
  ...
  else { ioppSort.key=c.k; ioppSort.dir=(c.t==='num'?-1:1); }
  ioppRenderHead(); ioppRenderRows();
}
function ioppRenderHead(){
  var el=document.getElementById('iopp-head'); if(!el) return;
  var h='';
  for(var i=0;i<IOPP_COLS.length;i++){
    var c=IOPP_COLS[i];
    var arr=(ioppSort.key===c.k)?('<span class="arr">'+(ioppSort.dir>0?'▲':'▼')+'</span>'):'';
    h+='<th class="sortable'+(c.c?' c':'')+'" onclick="ioppSortBy('+i+')">'+escapeHtml(c.label)+arr+'</th>';
  }
  el.innerHTML=h;
}
// Flatten every uploaded file into one sortable/filterable row list.
function ioppAllRows(){
  var fs=(IOPP&&IOPP.files)||[], out=[];
  for(var fi=0;fi<fs.length;fi++){
    var f=fs[fi], rws=f.rows||[], ven=f.vendor||f.name||'';
...
function ioppRenderRows(){
  var body=document.getElementById('iopp-tbody'); if(!body) return;
  var rows=ioppAllRows();
  if(ioppQ){
    rows=rows.filter(function(r){
      return (r.sku+' '+r.desc+' '+r.ven).toLowerCase().indexOf(ioppQ)>-1;
    });
  }
  var k=ioppSort.key, d=ioppSort.dir, col=null;
  for(var i=0;i<IOPP_COLS.length;i++){ if(IOPP_COLS[i].k===k) col=IOPP_COLS[i]; }
  rows.sort(function(a,b){
    var r=d*cmp(a[k],b[k],col?col.t:'str');
    return r!==0?r:cmp(a.sku,b.sku,'str');          // stable tiebreak on SKU
  });
  var total=rows.length, cap=2000, h='';
  for(var j=0;j<rows.length && j<cap;j++){
    var r2=rows[j];
    h+='<tr'+(r2.flag?' style="background:#f2fbf5;"':'')+'>'+
      '<td class="c">'+(r2.flag?'<span class="fop-dot"></span>':'')+'</td>'+
      '<td style="font-weight:600;">'+escapeHtml(r2.sku)+'</td>'+
      '<td>'+escapeHtml(r2.desc)+'</td>'+
      '<td class="c">'+fmtQty(r2.qty)+'</td>'+
...
// DIFFERENT VENDOR than the PO is placed with (drives the tab header count).
...
function renderIoppPanel(){
  ...
  // live in the Open Vendor POs tab's per-vendor INV columns, so no table is drawn here.
  var ms=ioppMatches();
  var sameV=fs.length?ioppSameVendorCount():0;

  // Shell only — the header row and the body are filled by ioppRenderHead() /
  // ioppRenderRows() so that typing in the search box never replaces the <input>.
  var invTbl = nRows
    ? ('<div class="iop-scroll"><table><thead><tr id="iopp-head"></tr></thead>'+
       '<tbody id="iopp-tbody"></tbody></table></div>'+
       '<div id="iopp-count" style="font-size:12px;color:#6b7a8a;padding:8px 2px;"></div>')
    : '';
  ...
  document.getElementById('panel').innerHTML =
    ...
    (nRows?('<h3 style="margin:18px 0 8px;color:#0D2B45;font-size:15px;">All uploaded inventory '+
      ...
      '<input id="iopp-q" type="search" placeholder="Search SKU, description or vendor…" '+
      'value="'+escapeHtml(ioppQ)+'" oninput="ioppSearch(this.value)" '+
      'style="padding:8px 10px;border:1px solid #cdd9e6;border-radius:8px;'+
      'font-size:13px;width:280px;margin-bottom:10px;">'+invTbl)
     :'<div class="empty">Upload an inventory file to get started.</div>');
  if(nRows){ ioppRenderHead(); ioppRenderRows(); }
  _ioppWireDrop();
}
...
function loadChealth(){
  ...
  fetch('customer-health-data.json?cb='+Date.now(),{cache:'no-store'})
    .then(function(r){ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
    .then(function(d){ CH=d; chLoading=false; if(mode==='chealth') renderChealthPanel(); })
...
function renderChealthPanel(){
  ...
  var labs=CH.labs||[], cnt=CH.counts||{}, sd=CH.stale_days||45;
  var shown=labs.filter(function(l){
  ...
  var bar=chip('all','All '+(cnt.total||labs.length),'')+
          chip('alert',(cnt.alert||0)+' alert','alert')+
          chip('watch',(cnt.watch||0)+' watch','watch')+
          chip('ok',(cnt.ok||0)+' clear','ok')+
          chip('due',(cnt.manual_due||0)+' re-check due','due');
  ...
  for(var i=0;i<shown.length;i++){
    ...
    body+='<tr class="row-'+escapeHtml(l.status)+'">'+
      ...
      '<td><ul class="ch-flags">'+flags+'</ul></td></tr>';
  }
  if(!shown.length) body='<tr><td colspan="7" class="ch-none" style="padding:22px;">Nothing in this bucket.</td></tr>';
  ...
  var html='<div class="panel-head"><h2>Customer Health</h2><div class="sub">'+
    ...
    '<div class="matrix-wrap"><table><thead><tr>'+
      ...
      '<td>Registry / courts</td><td>What was found</td>'+
    '</tr></thead><tbody>'+body+'</tbody></table></div>'+
