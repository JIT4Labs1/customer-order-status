...
function renderSpnlPanel(){
  ...
  var tRev=0,tCost=0,tPkg=0,tPoRows=0,nDisc=0,anyCost=false;
  for(var r=0;r<rows.length;r++){ tRev+=rows[r].revenue; tCost+=rows[r].cost; tPkg+=rows[r].packages; tPoRows+=rows[r].po_rows; if(rows[r].disc) nDisc++; if(rows[r].has_cost) anyCost=true; }
  ...
  for(var k=0;k<rows.length;k++){ var x=rows[k];
    ...
    var discDot = x.disc
      ? '<span title="Pkgs ('+x.packages+') ≠ PO Rows ('+x.po_rows+')" style="color:#c0392b;font-size:15px;">●</span>'
      : '<span title="Pkgs match PO Rows" style="color:#1e7d34;font-size:15px;">●</span>';
    body+='<tr>'+
