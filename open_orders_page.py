...
function shipItems(i){ var s=((SHIP&&SHIP.shipments)||[])[i]; if(!s) return; var it=s.items||[];
  ...
  // Sales Order # comes from the Shopify "Vtiger SO:" order tag; Purchase Order #(s) from Vtiger.
  var pos=s.pos||[]; if(!pos.length && s.po) pos=[{po:s.po,vendor:''}];
  var poHtml = pos.length
    ? pos.map(function(p){ return '<b>'+escapeHtml(p.po||'')+'</b>'+(p.vendor?' <span style="color:#5a6b7a;">('+escapeHtml(p.vendor)+')</span>':''); }).join(', ')
  ...
  var idLine =
    '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:10px;">'+
      ...
      '<span style="background:#eef3f9;border:1px solid #d5e0ec;border-radius:6px;padding:3px 10px;font-size:12.5px;">Purchase Order'+(pos.length>1?'s':'')+': '+poHtml+'</span>'+
    '</div>';
