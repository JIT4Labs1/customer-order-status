...
function cjWinTerms(){   // [{v,count}] search terms across base (window+outcome) sessions, most used first
  var ws=cjBaseSessions(), m={}, order=[];
...
function cjWinProducts(){   // [{v,count}] products added or purchased across base (window+outcome) sessions
  var ws=cjBaseSessions(), m={}, order=[];
...
function cjFacetBox(title, items, activeVal, fn, first){
  var top = first ? '' : 'margin-top:12px;border-top:1px solid #e3e8ee;padding-top:8px;';
  var h='<div style="'+top+'">'+
...
function renderCjTabs(el){
  ...
  var h=cjFacetBox('Products added / purchased', cjProdList, cjProdFilter, 'cjToggleProdIdx', true);
  h+=cjFacetBox('Search terms', cjTermList, cjTermFilter, 'cjToggleTermIdx', false);
  el.innerHTML=h;
}
var cjVisList=[];
