...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* ── Mobile (tablet & phone), app-like. All rules scoped to ≤760px so the
     ...
     screen (2.5% gutters); the mode nav is a wrapped tile grid (2–3 rows);
     data-dense tables scroll horizontally within the panel. ── */
  @media (max-width:760px){
    .layout{ flex-direction:column; padding:10px 2.5% 28px; }
    .sidecol{ flex-basis:auto; width:100%; }
    .tabs{ flex-basis:auto; width:100%; max-height:none; }
    .panel-wrap{ margin-left:0; margin-top:12px; }
    .header{ padding:14px 2.5%; }
    .header .brand{ font-size:19px; }
    .header .meta{ font-size:11px; }
    .kpis{ padding:12px 2.5% 0; gap:7px; }
    .kpi{ min-width:0; flex:1 1 30%; padding:10px 6px; }
    .kpi .v{ font-size:22px; }
    /* App-like mode nav: wrapped tile GRID (fills ~95% width, 2–3 rows) instead
       of a horizontal scroll strip. auto-fit keeps it 2 rows on tablets and
       ~3 rows on phones. */
    .modebar{ display:grid; grid-template-columns:repeat(auto-fit,minmax(78px,1fr));
      gap:6px; padding:12px 2.5%; border-bottom:none; overflow:visible; }
    .mode-btn{ margin:0; border:1px solid #cdd9e6; border-radius:10px; padding:9px 6px;
      font-size:11px; line-height:1.15; white-space:normal; text-align:center;
      min-height:46px; display:flex; align-items:center; justify-content:center; }
    .mode-btn.active{ box-shadow:0 1px 5px rgba(13,43,69,.28); }
    ...
    .ca-visuals{ gap:18px; padding:0 2.5%; }
    .footer{ padding:16px 2.5%; }
  ...
  @media (max-width:480px){
    .kpi{ flex:1 1 30%; padding:9px 4px; }
    .kpi .v{ font-size:20px; }
    .kpi .l{ font-size:10px; }
    /* keep the nav to ~3 rows on phones (10 tiles across 4 columns) */
    .modebar{ grid-template-columns:repeat(4,1fr); }
    .mode-btn{ font-size:10.5px; padding:8px 4px; min-height:44px; }
