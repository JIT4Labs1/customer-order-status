...
function gadsJourneyHtml(){
  ...
  // Only show landing pages for ENABLED campaigns (per the Google Ads status above).
  var enabled={}; var civId = ytd ? 'this_year' : 'last_30_days';
  var ivs=GADS.intervals||[]; for(var z=0;z<ivs.length;z++){ if(ivs[z].id===civId){ var cl=ivs[z].campaigns||[]; for(var y=0;y<cl.length;y++){ if((cl[y].status||'')==='enabled') enabled[cl[y].name]=1; } } }
  ...
  // Events / conversions by key event
  var evs=jiv.events||[], evb='';
  ...
  return '<div class="ca-h" style="margin-top:24px;border-top:1px solid #dee5ec;padding-top:14px;">User Journey (GA4) — paid Google Ads traffic &middot; '+jlabel+
    ...
    cards+
    '<div class="ca-h">Landing pages — enabled campaigns only, where ad clicks enter</div>'+
